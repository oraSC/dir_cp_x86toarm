...
int copy_file(unsigned char *src_path, unsigned char *dst_path)
{
	...
	struct stat *pstat_buf = &stat_buf;

	ret = stat(src_path, pstat_buf);
	if(ret < 0)
	{
	...
	ret = fwrite(pdata, size, 1, fd_dst);
	if(ret < 0)
	{
	...
	//修改用户权限
	ret = chmod(dst_path, pstat_buf->st_mode);
	if(ret < 0)
	{
